...
namespace NetJs.Translator.CSharpToJavascript
{
    ...
    public partial class SecondPassRewriter : CSharpSyntaxRewriter
    {
        ...
        private readonly List<InvocationExpressionSyntax> _pendingInvocationExpressions = new();
        ...
        //{
        //    return TryGetQueryMemberAccessForVariable(node);
        //}
        ...
        private ExpressionSyntax? TryGetQueryMemberAccessForVariable(IdentifierNameSyntax variable)
        {
            int depth;
            if (!_state.IsAnonymousType ||
                !_state.IdentifiersChain.TryGetValue(variable.Identifier.ValueText, out depth))
                return null;
            var rangeVariable = GetSymbol(variable) as IRangeVariableSymbol;
        ...
        private SyntaxToken GetRangeVariable(ExpressionSyntax node)
        {
            try
            {
                if (_semanticModel.SyntaxTree == node.SyntaxTree)
                {
                    return Identifier(
                        node.DescendantNodesAndSelf().
                        Select(n => _semanticModel.GetSymbolInfo(n).Symbol).
        ...
        private IEnumerable<SyntaxToken> GetRangeVariables(params SyntaxNode[] nodes)
        {
            return (from node in nodes
                    ...
                    select _semanticModel.GetSymbolInfo(child).Symbol ??
                           _semanticModel.GetDeclaredSymbol(child) into symbol
